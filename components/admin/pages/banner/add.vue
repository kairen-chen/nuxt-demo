    ...
    <Layout class="main">
        <div class="add-form">
            ...
            <div class="add-form-content">
                <div class="form-head">
                    <div class="form-head-title">新增圖片</div>
                </div>
                ...
                <Form id="addFormValidate" ref="addFormValidate" :model="addFormValidate" :rules="addRuleValidate" label-position="top">
                    ...
                    <Row>
                        <i-Col span="12">
                            <FormItem class="add-form-item" label="圖片網址" prop="url">
                                <Input v-model.trim="addFormValidate.url" maxlength="64"></Input>
                            </FormItem>
                        </i-Col>
                    </Row>
                    <Row>
                        <i-Col span="24">
                            <FormItem class="add-form-item" label="宣傳圖片" prop="mapPath">
                                    <!-- <a v-show="!updSrc" :href="addFormValidate.image" target="_blank"><img :src="addFormValidate.image" class="previewPlanes"></a> -->
                                    <div class="upd-img" v-show="updSrc">
                                        <img class="previewPlanes" :src="updSrc" alt="">
                                    </div>                                    
                                    <Upload ref="uploadImg" v-model="addFormValidate.mapPath" action="" :show-upload-list="false" accept="image/png, image/jpeg" :before-upload="handleUpload">
                                        <Button icon="ios-cloud-upload-outline">更新檔案</Button>
                                        <!-- <div class="upload-c">
                                            <Icon type="ios-cloud-upload" size="52" style="color: #3399ff"></Icon>
                                            <div class="upload-txt">選擇或拖拉圖片</div>
                                        </div> -->
                                    </Upload>
                            </FormItem>
                    ...
                    <Row>
                        <i-Col span="24">
                            <FormItem class="add-form-item" label="手機版宣傳圖片" prop="verifiedImage">
                                <!-- <a v-show="!VerifiedImageUpdSRC" :href="addFormValidate.verifiedImage" target="_blank"><img :src="addFormValidate.verifiedImage" class="previewPlanes"></a> -->
                    ...
                    <Row>
                        <i-Col span="12">
                            <FormItem class="add-form-item" label="排序(依數字由大至小顯示)" prop="displaySort">
                                <Input type="number" v-model.number="addFormValidate.displaySort"></Input>
                            </FormItem>
...
export default {
    ...
    data() {
        const validateUpload = (rule, value, callback) => {
            if (this.addFormValidate.mapPath === null) {
        ...
        const validateMobileUpload = (rule, value, callback) => {
            if (this.addFormValidate.verifiedImage === null) {
    ...
    mounted() {
        ...
        var form = document.querySelector('#addFormValidate');
        let _this = this
    ...
    methods: {
        ...
        addHandleSubmit(name) {
            this.$refs[name].validate((valid) => {
                if (valid) {
                    ...
                    data.push({
                        url: this.addFormValidate.url,
                        displaySort: this.addFormValidate.displaySort,
                    })
                    // console.log(data)
                    let _this = this
                    let errorDesc = ''
                    this._API.postBanner.send(data).then((data) => {
        ...
        handleUpload(file) { // 上传文件前的事件钩子
            ...
            if (file.size >= (this.maxSize * 1024000)) {
                ...
                this.addFormValidate.mapPath = file
                console.log('this.addFormValidate.mapPath:' + this.addFormValidate.mapPath)
                // 选择文件后触发验证关闭错误提示
        ...
        handleUploadVerifiedImage(file) { // 上传文件前的事件钩子
            ...
            if (file.size >= (this.maxSize * 1024000)) {
                ...
                this.addFormValidate.verifiedImage = file
        ...
        removeFile(keyID) {
            ...
            this.updSrc = ""
            this.addFormValidate.mapPath = null
        },
        handleMaxSize(file) {
            this.$Notice.warning({
