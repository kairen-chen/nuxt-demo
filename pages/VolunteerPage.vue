...
export default {
  ...
  computed: {
    ...mapGetters(["bannerDataGetter", "volunteerEventDataGetter"]),
  },
  watch: {
    bannerDataGetter: {
      immediate: true,
    ...
    volunteerEventDataGetter: {
      immediate: true,
      handler() {
        if (this.volunteerEventDataGetter !== null) {
          this.count = this.volunteerEventDataGetter.count;
          this.volunteerEventDataGetter.results.forEach((item) => {
  ...
  async asyncData(context) {
    return Promise.all([
      ...
      context.store.dispatch("getVolunteerEvent", {
        page: 0,
        size: 12,
